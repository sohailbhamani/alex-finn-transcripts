# Graph Report - /mnt/data/dev/alex-finn-transcripts/.  (2026-04-10)

## Corpus Check
- Large corpus: 349 files · ~1,763,480 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.
...
- `enrich_transcript()` --calls--> `extract_entities()`  [INFERRED]
  /mnt/data/dev/alex-finn-transcripts/scripts/enrich.py → /mnt/data/dev/alex-finn-transcripts/scripts/enrich.py  _Bridges community 12 → community 6_
- `enrich_transcript()` --calls--> `classify_topic()`  [INFERRED]
  /mnt/data/dev/alex-finn-transcripts/scripts/enrich.py → /mnt/data/dev/alex-finn-transcripts/scripts/enrich.py  _Bridges community 9 → community 6_
- `enrich_transcript()` --calls--> `classify_difficulty()`  [INFERRED]
  /mnt/data/dev/alex-finn-transcripts/scripts/enrich.py → /mnt/data/dev/alex-finn-transcripts/scripts/enrich.py  _Bridges community 8 → community 6_
- `enrich_transcript()` --calls--> `classify_audience()`  [INFERRED]
  /mnt/data/dev/alex-finn-transcripts/scripts/enrich.py → /mnt/data/dev/alex-finn-transcripts/scripts/enrich.py  _Bridges community 11 → community 6_
- `enrich_transcript()` --calls--> `extract_concepts()`  [INFERRED]
  /mnt/data/dev/alex-finn-transcripts/scripts/enrich.py → /mnt/data/dev/alex-finn-transcripts/scripts/enrich.py  _Bridges community 5 → community 6_
...
Cohesion: 0.5
Nodes (3): extract_concepts(), main(), Extract key concepts from text.
...
- **Thin community `Community 9`** (2 nodes): `classify_topic()`, `Classify primary topic.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 10`** (2 nodes): `generate_summary()`, `Generate bullet point summary.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `enrich_transcript()` connect `Community 6` to `Community 5`, `Community 8`, `Community 9`, `Community 10`, `Community 11`, `Community 12`?**
